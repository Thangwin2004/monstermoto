...
export type PickupType = 'buff_rapid' | 'buff_shield' | 'buff_heal' | 'buff_nuke' | 'star_upgrade';
...
export class Pickup extends Container {
    ...
    private renderCrate(cfg: PickupConfig) {
        ...
        // 1. Soft Shadow
        this.crateGfx.roundRect(-22, -18, 44, 44, 12)
            .fill({ color: 0x000000, alpha: 0.35 });

        // 2. 3D Crate Body
        this.crateGfx.roundRect(-20, -20 + 4, 40, 40, 10)
            .fill(cfg.shadowColor);
        this.crateGfx.roundRect(-20, -20, 40, 40, 10)
            .fill(cfg.color)
            .stroke({ color: 0xffffff, width: 2.5 });

        // 3. Top Gloss Sheen
        this.crateGfx.roundRect(-16, -18, 32, 16, 6)
    ...
    updatePickup(dt: number, convoyX: number, convoyY: number, roadSpeed: number): boolean {
        ...
        this.auraGfx.circle(0, 0, 26 * pulse)
