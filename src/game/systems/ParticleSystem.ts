...
export class ParticleSystem {
    ...
    explode(x: number, y: number, count: number = 20, color: number = 0xff6600) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const pColor = Math.random() < 0.35 ? 0xffffff : (Math.random() < 0.5 ? 0xffcc00 : color);
    ...
    hitSpark(x: number, y: number, color: number = 0xffea00, count: number = 6, baseVx: number = 0, baseVy: number = 0) {
    ...
    bloodSplatter(x: number, y: number, color: number = 0xef4444, count: number = 5) {
        for (let i = 0; i < count; i++) {
            const angle = Math.random() * Math.PI * 2;
    ...
    lightningBolt(x1: number, y1: number, x2: number, y2: number, color: number = 0x00e5ff) {
    ...
    update(dt: number) {
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            if (p.shape === 'ring') {
                this.gfx.circle(p.x, p.y, curSize)
                ...
                this.gfx.circle(p.x, p.y, Math.max(1, curSize))
