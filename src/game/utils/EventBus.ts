...
export type GameEventMap = {
    'enemy:killed': { x: number; y: number; enemyType: string; hasBurn?: boolean; hasShock?: boolean };
    'enemy:hit': { enemyId: number; damage: number; weaponType: string };
    'module:damaged': { moduleIndex: number; damage: number; currentHp: number; maxHp: number };
    'module:destroyed': { moduleIndex: number; moduleId: string; x: number; y: number };
    ...
    'scrap:collected': { amount: number; totalScrap: number; x?: number; y?: number };
    'run:ended': { victory: boolean; distance: number; kills: number; score: number };
    ...
    'damage:number': { x: number; y: number; amount: number; crit?: boolean; heal?: boolean; status?: 'burn' | 'shock' | 'crit' };
};
