...
import { ROAD_SPEED, XP_PER_LEVEL_BASE, XP_PER_LEVEL_GROWTH } from '../constants';
...
export class LootSystem {
    ...
    getXpRequired(): number {
        return (XP_PER_LEVEL_BASE * 1.1) + (this.level - 1) * (XP_PER_LEVEL_GROWTH * 1.1);
    }
    ...
    update(dt: number, convoyX: number, convoyY: number, onCollect: (type: PickupType) => void) {
        const dtSec = dt * (1 / 60);
