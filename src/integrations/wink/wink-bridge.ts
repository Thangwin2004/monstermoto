...
export function getPersonalBest(): Promise<PersonalBestResponse> {
  return Promise.resolve()
    .then(() => {
      const bridge = getWinkBridge();
      ...
      if (typeof bridge.getLeaderboard === 'function') {
        return bridge.getLeaderboard({ limit: 1 }).then((res) => ({ me: res?.me ?? null }));
      }
      return { me: null };
    })
    .catch((err) => {
  ...
  return Promise.resolve()
    .then(() => {
      ...
      if (!bridge || !bridge.getCapabilities?.()?.submitScore) {
        return { entry: null as any, isNewBest: false, previousBest: null };
      }
