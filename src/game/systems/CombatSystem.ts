 ...
 */
function getModulePos(convoyX: number, convoyY: number, module: Module): { x: number; y: number } {
    return {
        x: convoyX + module.x,
        y: convoyY + module.y,
    };
...
export class CombatSystem {
        ...
        for (const module of convoy.modules) {
            ...
            if (flameLvl > 0) {
                ...
                if (module.flameCooldownTimer <= 0) {
                    ...
                    const flameDmg = Math.round((8 + flameLvl * 6) * module.stats.damageMultiplier);
                    ...
                    this.projectileSystem.spawn(mPos.x - 34, mPos.y - 10, -50 + (Math.random() - 0.5) * 40, -520, flameDmg, 'flame', {
                        aoeRadius: aoe,
            ...
            if (teslaLvl > 0) {
                ...
                if (module.teslaCooldownTimer <= 0) {
                    ...
                    const teslaDmg = Math.round((28 + teslaLvl * 16) * module.stats.damageMultiplier);
                    ...
                    if (teslaTarget) {
                        ...
                        this.particleSystem.lightningBolt(leftCoilX, coilY, teslaTarget.x, teslaTarget.y, 0x00f0ff);
                        this.particleSystem.lightningBolt(rightCoilX, coilY, teslaTarget.x, teslaTarget.y, 0x38bdf8);

                        // Find primary enemy target
                        const primaryEnemy = this.enemySystem.enemies.find(e => e.active && distance(e.x, e.y, teslaTarget.x, teslaTarget.y) < 25);
                        if (primaryEnemy) {
                            const killed = primaryEnemy.takeDamage(teslaDmg);
                            primaryEnemy.applyShock(2.5);
                            this.particleSystem.hitSpark(primaryEnemy.x, primaryEnemy.y, 0x00ffff, 8);
                            EventBus.emit('damage:number', { x: primaryEnemy.x, y: primaryEnemy.y - 20, amount: teslaDmg, status: 'shock' });
                            ...
                            for (const other of this.enemySystem.enemies) {
                                ...
                                if (distance(lastX, lastY, other.x, other.y) < 240) {
                                    this.particleSystem.lightningBolt(lastX, lastY, other.x, other.y, 0x67e8f9);
                                    ...
                                    EventBus.emit('damage:number', { x: other.x, y: other.y - 20, amount: chainDmg, status: 'shock' });
                        ...
                        this.particleSystem.lightningBolt(leftCoilX, coilY, mPos.x - 20 + (Math.random() - 0.5) * 40, mPos.y - 220, 0x00f0ff);
                        this.particleSystem.lightningBolt(rightCoilX, coilY, mPos.x + 20 + (Math.random() - 0.5) * 40, mPos.y - 220, 0x38bdf8);
                    }
                }
            }
        }
    ...
    private fireWeaponForward(
        ...
        AudioMixer.playShoot(projType === 'flame' ? 'flame' : projType === 'lightning' ? 'tesla' : projType === 'rocket' ? 'rocket' : 'bullet');

        // Straight forward direction (-90 deg = -Math.PI / 2)
        const mainAngle = -Math.PI / 2;

        // Muzzle flash particle
        this.particleSystem.muzzleFlash(from.x, from.y - 24, mainAngle, module.data.color);
        ...
        for (let i = 0; i < projCount; i++) {
            ...
            let isCrit = false;
            if (module.stats.critChance > 0 && gameRng.chance(module.stats.critChance)) {
                finalDamage = Math.round(baseDamage * module.stats.critMultiplier);
                isCrit = true;
            }

            const spawnedP = this.projectileSystem.spawn(from.x, from.y - 24, vx, vy, finalDamage, projType, {
    ...
    private findTarget(fromPos: { x: number; y: number }, range: number): { x: number; y: number } | null {
        ...
        for (const p of this.projectileSystem.projectiles) {
            ...
            for (const e of this.enemySystem.enemies) {
                ...
                if (circlesOverlap(p.x, p.y, p.radius, e.x, e.y, e.radius)) {
                    ...
                    const sparkColor = p.projType === 'flame' ? 0xff6600 : p.projType === 'lightning' ? 0x00ffff : 0xffea00;
                    this.particleSystem.hitSpark(p.x, p.y, sparkColor, isCrit ? 10 : 6, p.vx * 0.15, p.vy * 0.15);
                    ...
                    } else if (p.projType === 'flame' || p.aoeRadius > 0 || p.burnChance > 0) {
    ...
    private handleSplashDamage(x: number, y: number, radius: number, damage: number) {
    ...
    private handleProjectileVsBoss() {
        ...
        for (const p of this.projectileSystem.projectiles) {
            ...
            if (circlesOverlap(p.x, p.y, p.radius, bx, by, bossRadius)) {
                ...
                this.particleSystem.hitSpark(p.x, p.y, 0xffaa00, 8, p.vx * 0.15, p.vy * 0.15);
    ...
    private handleEnemyVsConvoy() {
        ...
        for (const e of this.enemySystem.enemies) {
            ...
            for (const m of convoy.modules) {
                ...
                if (circlesOverlap(mPos.x, mPos.y, MODULE_COLLISION_RADIUS, e.x, e.y, e.radius)) {
    ...
    private handleEnemyProjectilesVsConvoy() {
        ...
        for (const p of this.projectileSystem.projectiles) {
            ...
            for (const m of convoy.modules) {
                ...
                const mPos = getModulePos(convoy.x, convoy.y, m);
                if (circlesOverlap(p.x, p.y, p.radius, mPos.x, mPos.y, MODULE_COLLISION_RADIUS)) {
    ...
    private onEnemyKilled(enemy: Enemy) {
        ...
        this.lootSystem.onEnemyDefeated(enemy.x, enemy.y, enemy.archetype === 'tank' || enemy.archetype === 'colossus');
