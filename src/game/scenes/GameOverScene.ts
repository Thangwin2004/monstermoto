...
export class GameOverScene extends Container implements Scene {
    ...
    constructor() {
        ...
        cardShadow.roundRect(-cardW / 2 + 8, -cardH / 2 + 16, cardW, cardH, 28)
        ...
        ribbon.roundRect(-ribbonW / 2 + 16, ribbonY + 4, ribbonW - 32, ribbonH * 0.38, 12)
        ...
        // 6. 3D Marth3 Buttons
        this.playAgainBtn = this.createMarth3Button('CHƠI LẠI', 0xf59e0b, 0xb45309, 0, () => {
            SceneManager.switchScene('RunScene');
        });
        this.playAgainBtn.y = GAME_HEIGHT * 0.82;
        this.addChild(this.playAgainBtn);

        const menuBtn = this.createMarth3Button('TRANG CHỦ', 0x0284c7, 0x0369a1, 0, () => {
    ...
    private buildStatsList() {
        ...
        const stats = [
            { label: 'Quãng đường', value: `${Math.floor(rs.distance)} m`, icon: '📏' },
            ...
            { label: 'Thời gian sinh tồn', value: `${Math.floor(rs.runTime / 60)}:${String(Math.floor(rs.runTime % 60)).padStart(2, '0')}`, icon: '⏱' },
        ];
        ...
        for (let i = 0; i < stats.length; i++) {
            ...
            rowBg.roundRect(-240, y - 20, 480, 42, 10)
        ...
        scoreBox.roundRect(-240, scoreBoxY - 24, 480, 72, 14)
            .fill(0xfef3c7)
    ...
    private createMarth3Button(label: string, colorTop: number, colorShadow: number, y: number, onClick: () => void): Container {
        ...
        shadow.roundRect(-w / 2, -h / 2 + 8, w, h, radius)
            .fill(colorShadow);
        btn.addChildAt(shadow, 0);

        // Body
        const body = new Graphics();
        body.roundRect(-w / 2, -h / 2, w, h, radius)
            .fill(colorTop)
            .stroke({ color: 0xffffff, width: 4 });
        body.roundRect(-w / 2 + 12, -h / 2 + 4, w - 24, h * 0.38, 14)
            .fill({ color: 0xffffff, alpha: 0.3 });
