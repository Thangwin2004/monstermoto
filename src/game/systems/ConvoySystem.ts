...
import {
    GAME_WIDTH, CONVOY_Y, CONVOY_MOVE_SPEED,
    CONVOY_CLAMP_MARGIN, ROAD_LEFT, ROAD_RIGHT,
} from '../constants';
...
export class ConvoySystem {
    ...
    private setupInput(parent: Container) {
        ...
        parent.on('pointermove', (e: FederatedPointerEvent) => {
            ...
            this.targetX = Math.max(ROAD_LEFT + CONVOY_CLAMP_MARGIN, Math.min(ROAD_RIGHT - CONVOY_CLAMP_MARGIN, this.targetX));
        });

        parent.on('pointerup', () => { this.isDragging = false; });
        parent.on('pointerupoutside', () => { this.isDragging = false; });
    }
    ...
    update(dt: number) {
        ...
        this.convoy.x = moveTowards(this.convoy.x, this.targetX, CONVOY_MOVE_SPEED * dtSec);
