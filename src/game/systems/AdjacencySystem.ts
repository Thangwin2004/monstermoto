...
export class AdjacencySystem {
    ...
    private getTargetModules(convoy: Convoy, sourceIndex: number, effect: AdjacencyEffect): Module[] {
        ...
        switch (effect.target) {
            case 'front':
                if (front && !front.isDead) {
                    if (!effect.requireTag || front.data.tags.includes(effect.requireTag)) {
                        results.push(front);
                    }
                }
                break;
            ...
            case 'behind':
                if (behind && !behind.isDead) {
                    if (!effect.requireTag || behind.data.tags.includes(effect.requireTag)) {
                        results.push(behind);
                    }
                }
                break;
    ...
    private applyEffect(module: Module, effect: AdjacencyEffect) {
        switch (effect.stat) {
            ...
            case 'attackSpeed':
                if (effect.isMultiplier) module.stats.attackSpeedMultiplier *= effect.value;
                else module.stats.attackSpeedMultiplier += effect.value;
                break;

            case 'damageTaken':
                if (effect.isMultiplier) module.stats.damageTakenMultiplier *= effect.value;
