...
export class EnemySystem {
  ...
  update(dt: number) {
    ...
    const targetY = frontModule ? convoy.y + frontModule.y : convoy.y;
  ...
  private startEncounter() {
    ...
    this.activeWaves = encounter.waves.map((w) => {
      const scaledCount = Math.max(
        1,
        Math.round(w.count * this.densityMultiplier),
      );
      return {
        entry: w,
        spawned: 0,
  ...
  private processWaves(dtSec: number) {
    for (let i = this.activeWaves.length - 1; i >= 0; i--) {
      ...
      if (wave.timer <= 0 && wave.spawned < wave.totalCount) {
        ...
        wave.timer = Math.max(
          0.08,
          (wave.entry.interval ?? 0.5) / Math.min(2.0, this.densityMultiplier),
        );
      }
