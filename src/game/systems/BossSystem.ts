...
export class BossSystem {
  ...
  // State
  private state:
    "idle" | "approach" | "combat" | "hooking" | "hooked" | "defeated" = "idle";
  ...
  private renderWarRig() {
    ...
    // Massive Shadow
    this.shadow.ellipse(0, 50, 120, 50).fill({ color: 0x000000, alpha: 0.5 });
    ...
    g.roundRect(-112, -35, 20, 70, 4)
      .fill(0x3f3f46)
      .stroke({ color: 0x000000, width: 2.5 });
    g.roundRect(92, -35, 20, 70, 4)
      .fill(0x3f3f46)
      .stroke({ color: 0x000000, width: 2.5 });
    ...
    g.roundRect(-30, -35, 60, 35, 8)
      .fill(0x18181b)
      .stroke({ color: 0x000000, width: 2 });
    g.ellipse(-14, -20, 10, 5)
      .fill(0xf97316)
  ...
  private updateHpBar() {
    ...
    this.hpBarFill.roundRect(-78, -88, fillW, 8, 4).fill(0xff3b30);
  }
  ...
  private renderAimLaser() {
    ...
    this.laserGfx
      .moveTo(this.container.x, this.container.y + 40)
      .lineTo(convoy.x, convoy.y)
      .stroke({
        color: 0xff0000,
        width: 2,
        alpha: 0.5 + Math.sin(this.animTime * 20) * 0.4,
      });
  }
  ...
  private drawChainAndHook(x1: number, y1: number, x2: number, y2: number) {
    ...
    this.hookGraphics
      .moveTo(x2, y2)
      .lineTo(
        x2 + Math.cos(hookAngle + 0.8) * 22,
        y2 + Math.sin(hookAngle + 0.8) * 22,
      )
